package com.smart.uicore.controls.support
{
	...
	public class ItemsSelectControl
	{
		...
		public function initViewPanel(viewBox:List,itemsBox:Sprite):void
		{
			view = viewBox;
			items = itemsBox;
		...
		private function startShowBox(event:MouseEvent):void
		{
			if(view == null) return;
			if(view.allowMultipleSelection == false) return;
			if(view.mouseX > view.width-VScrollBar.defaultWidth-5) return;
		...
		private function clearShow(event:MouseEvent):void
		{
		...
		private function showBox(event:MouseEvent=null):void
		{
		...
		private function drawBox(event:MouseEvent):void
		{
			...
			var y1:Number = Math.min(p1.y, p2.y);
			var y2:Number = Math.max(p1.y, p2.y);
			if(x2>view.compoWidth) x2 = view.compoWidth;
			if(y2>view.compoHeight) y2 = view.compoHeight;
			sp.graphics.clear();
			sp.graphics.beginFill(SkinThemeColor.listBackgroundFillColors[SkinThemeColor.listBackgroundFillColors.length-1], 0.4);
			sp.graphics.lineStyle(0.1, SkinThemeColor.listBackgroundFillColors[SkinThemeColor.listBackgroundFillColors.length-1],1.0);
